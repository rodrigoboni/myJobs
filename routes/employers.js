...
var Model = require('../models/employers.js');
...
router.post('/', (req, res, next) => {
  ...
  Events.once('create', (err, result) => {
    ...
    if (err) {
      ...
      res.status(500).json({ message: 'Could not create employer', err: err });
    }
...
router.get('/', (req, res, next) => {
  ...
  Events.once('getAll', (err, result) => {
    ...
    if (err) {
      console.log('Response', { err });
      res.status(500).json({ message: 'Could not retrieve employers', err: err });
    }
...
router.get('/:id', (req, res, next) => {
  ...
  Events.once('get', (err, result) => {
    ...
    if (err) {
      console.log('Response', { err });
      res.status(500).json({ message: `Could not retrieve employer ${id}`, err: err });
    }
...
router.put('/:id', (req, res, next) => {
  ...
  Events.once('update', (err, result) => {
    ...
    if (err) {
      console.log('Response', { err });
      res.status(500).json({ message: `Could not udpate employer ${id}`, err: err });
    }
...
router.delete('/:id', (req, res, next) => {
  ...
  Events.once('delete', (err, result) => {
    ...
    if (err) {
      console.log('Response', { err });
      res.status(500).json({ message: `Could not delete employer ${id}`, err: err });
    }
